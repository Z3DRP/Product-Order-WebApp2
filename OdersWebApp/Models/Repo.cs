...
namespace OdersWebApp.Models
{
    ...
    {
        protected OrderContext context { get; set; }
        private DbSet<T> dbset { get; set; }
        public Repo(OrderContext ctx)
        {
            context = ctx;
            dbset = context.Set<T>();
        }

        // get number of retrieved entities use private backing field
        // bc when filtering count might be less than dbSet.Count()
        private int? count;
